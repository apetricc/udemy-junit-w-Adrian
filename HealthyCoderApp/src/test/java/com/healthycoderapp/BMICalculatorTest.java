...
class BMICalculatorTest {
	...
	//setting an 'environment' flag to denote a particular environment
	...
	/**
	 * Junit allows 'nesting' of tests to keep things organized, 
	 * we just use an inner class to keep things in order, you can even go so far as to 
	 * create inner classes inside inner classes etc; 
	 * This is relevant when you have a bunch of tests and want to keep them organized; 
	 */
...
//			);   
		...
		//here we will take into account our environment
		@Test
		void should_ReturnCoderWithWorstBMIin1Ms_When_CoderListHas10000Elements_InProd_Environment() {
			//given
			//assumeTrue will only get skipped if not true, not failed
			assumeTrue(super.equals("prod"));
			List<Coder> coders = new ArrayList<>();
			for (int i = 0; i < 10000; i++) {
				coders.add(new Coder(1.0 + i, 10.0 + i));
			}
